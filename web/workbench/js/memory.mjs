...
/* S-layer topics this view reads, held to the manifest at build time by
   a test that reads this table and at run time by the screen's own
   asking. The stream table is polled apart from the tables it points at
   because a fault quarantines a stream, and the sync stamps date the
   root a live answer walked from. */
