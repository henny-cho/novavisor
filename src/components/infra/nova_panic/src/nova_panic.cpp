...
// with the first-failure report. The name is stdx's, so the project's
// g_ prefix for globals cannot apply.
template <>
...
// declaration; the body never returns either way.
void __glibcxx_assert_fail(const char* file, int line, const char* function, const char* condition) noexcept {
  nova::panic::fail("libstdc++ assertion failed: ", condition != nullptr ? condition : "?",
                    "\n  in: ", function != nullptr ? function : "?", "\n  at: ", file != nullptr ? file : "?", ":",
                    nova::console::Dec{static_cast<std::uint64_t>(line)});
}
