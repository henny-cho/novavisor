...
// a constant offset): chunks whose IPA slot
...
// Table set for one Stage 2 address space: the L1 root plus pools of L2
// and L3 tables consumed on demand.
//
...
// sentinels. Each *_pool_pas entry must be the PA of its corresponding
// table and must be 4 KiB-aligned.
struct Stage2Tables {
  Table*                         l1;
  std::span<Table>               l2_pool;
  std::span<const std::uint64_t> l2_pool_pas;
  std::span<Table>               l3_pool;
  std::span<const std::uint64_t> l3_pool_pas;
  std::size_t                    l2_used = 0;
...
inline auto find_l2(Stage2Tables& t, std::uint64_t table_pa) noexcept -> Table* {
  for (std::size_t i = 0; i < t.l2_used; ++i) {
    if ((t.l2_pool_pas[i] & desc::kOutputAddrMask) == table_pa) {
      return &t.l2_pool[i];
    }
  }
  return nullptr;
...
//   - the IPA or PA range exceeds the configured address width
//   - an L2 or L3 pool is exhausted
...
[[nodiscard]] inline auto map_range(Stage2Tables& t, std::uint64_t ipa_base, std::uint64_t pa_base, std::uint64_t size,
  ...
  constexpr std::uint64_t kIpaLimit = 1ULL << 39U;
  constexpr std::uint64_t kPaLimit  = 1ULL << 40U;
  if (t.l1 == nullptr || t.l2_pool.size() != t.l2_pool_pas.size() || t.l3_pool.size() != t.l3_pool_pas.size() ||
      size == 0 || (ipa_base % k4KiB) != 0 || (pa_base % k4KiB) != 0 || (size % k4KiB) != 0 || ipa_base >= kIpaLimit ||
      size > kIpaLimit - ipa_base || pa_base >= kPaLimit || size > kPaLimit - pa_base) {
    return false;
  }
  ...
  Table&              l1  = *t.l1;
  const std::uint64_t end = ipa_base + size;

  std::uint64_t addr = ipa_base;
  while (addr < end) {
    const std::size_t i1 = l1_index(addr);
    Table*            l2 = nullptr;
    if (!is_valid(l1[i1])) {
      if (t.l2_used >= t.l2_pool.size()) {
        return false;
      }
      l1[i1] = make_table(t.l2_pool_pas[t.l2_used]);
      l2     = &t.l2_pool[t.l2_used++];
    } else {
      if (descriptor_type(l1[i1]) != desc::kTypeTable) {
        return false;
      }
      l2 = detail::find_l2(t, output_addr(l1[i1]));
      if (l2 == nullptr) {
        return false;
      }
    }
    const std::uint64_t l1_end     = ((addr >> kL1Shift) + 1U) << kL1Shift;
    const std::uint64_t region_end = end < l1_end ? end : l1_end;
    ...
    while (addr < region_end) {
      ...
      if ((addr % k2MiB) == 0 && ((addr + pa_off) % k2MiB) == 0 && region_end - addr >= k2MiB && !is_valid((*l2)[i2])) {
      ...
      const std::uint64_t chunk_end = (region_end < slot_end) ? region_end : slot_end;
      for (; addr < chunk_end; addr += k4KiB) {
        const std::size_t i3 = l3_index(addr);
        if (is_valid((*l3)[i3])) {
          return false;
        }
        (*l3)[i3] = make_page(addr + pa_off, leaf_attrs);
      }
    }
  }
  return true;
