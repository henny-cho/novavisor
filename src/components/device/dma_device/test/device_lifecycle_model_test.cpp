...
// ---------------------------------------------------------------------------
// Lifecycle walk classification — the quiesce/resume transition table
// ---------------------------------------------------------------------------

TEST(DeviceLifecycle, BeginQuiesceClassification) {
  using nova::dma_device::classify_begin_quiesce;
  using nova::dma_device::ScanAction;
  using nova::dma_device::State;

  EXPECT_EQ(classify_begin_quiesce(State::kUnavailable), ScanAction::kSkip);
  EXPECT_EQ(classify_begin_quiesce(State::kQuiesced), ScanAction::kSkip);
  EXPECT_EQ(classify_begin_quiesce(State::kQuiescing), ScanAction::kPending);
  EXPECT_EQ(classify_begin_quiesce(State::kDetaching), ScanAction::kPending);
  EXPECT_EQ(classify_begin_quiesce(State::kActive), ScanAction::kCollect);
  EXPECT_EQ(classify_begin_quiesce(State::kFailed), ScanAction::kFail);
  EXPECT_EQ(classify_begin_quiesce(State::kResuming), ScanAction::kFail);
}

TEST(DeviceLifecycle, QuiescingWalkClassification) {
  using nova::dma_device::classify_quiescing;
  using nova::dma_device::ScanAction;
  using nova::dma_device::State;

  EXPECT_EQ(classify_quiescing(State::kUnavailable, true), ScanAction::kSkip);
  EXPECT_EQ(classify_quiescing(State::kQuiesced, true), ScanAction::kSkip);
  // Only a fully bus-master-blocked quiescing entry advances.
  EXPECT_EQ(classify_quiescing(State::kQuiescing, true), ScanAction::kCollect);
  EXPECT_EQ(classify_quiescing(State::kQuiescing, false), ScanAction::kPending);
  EXPECT_EQ(classify_quiescing(State::kDetaching, true), ScanAction::kPending);
  EXPECT_EQ(classify_quiescing(State::kResuming, true), ScanAction::kPending);
  EXPECT_EQ(classify_quiescing(State::kActive, true), ScanAction::kPending);
  EXPECT_EQ(classify_quiescing(State::kFailed, true), ScanAction::kFail);
}

TEST(DeviceLifecycle, ResumeClassification) {
  using nova::dma_device::classify_resume;
  using nova::dma_device::ScanAction;
  using nova::dma_device::State;

  EXPECT_EQ(classify_resume(State::kUnavailable), ScanAction::kSkip);
  EXPECT_EQ(classify_resume(State::kQuiesced), ScanAction::kCollect);
  EXPECT_EQ(classify_resume(State::kQuiescing), ScanAction::kFail);
  EXPECT_EQ(classify_resume(State::kDetaching), ScanAction::kFail);
  EXPECT_EQ(classify_resume(State::kResuming), ScanAction::kFail);
  EXPECT_EQ(classify_resume(State::kActive), ScanAction::kFail);
  EXPECT_EQ(classify_resume(State::kFailed), ScanAction::kFail);
}
