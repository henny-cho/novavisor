...
struct boot_order_component {
  constexpr static auto config = cib::config(cib::extend<cib::RuntimeStart>(
      ...
      soft_timer_component::INIT >> core_vcpu_component::INIT >> boot_msg_component::PRINT_BOOT_MSG));
};

struct nova_project {
  constexpr static auto config = cib::components<core_mmu_component, core_gic_component, vgic_component,
                                                 core_timer_component, soft_timer_component, boot_msg_component,
                                                 trap_handler_component, core_vcpu_component, boot_order_component>;
};
