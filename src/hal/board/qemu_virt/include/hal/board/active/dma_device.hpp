...
namespace nova::board::active::pci_edu {
...
// The guest-visible BAR0 window and physical SPI are ABI (nova/abi/
// guest_layout.h) — the passthrough mapping and the vIRQ route both
// depend on these exact values, so derive rather than restate them.
inline constexpr std::uint64_t kBar0Size       = NOVA_EDU_BAR0_SIZE;
inline constexpr std::uint32_t kPhysicalIntid  = NOVA_EDU_SPI;
