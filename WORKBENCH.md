...
`SIGINT` or `SIGTERM` to a backgrounded bridge (`kill -INT <pid>`) — both walk
the same teardown, also for a bridge started under `nohup`, `setsid` or a
supervisor, where the shell hands the child SIGINT ignored. Shutdown tears the
whole session down: the QEMU child is terminated and the observation surfaces under
