...
config YAML (`configs/*.yml`, selected with `scripts/nova build --config`;
...
4. Once `./scripts/nova demo verify NN_name` passes locally, flip `enabled: true` in the manifest. That commit marks the phase as complete.

## How the harness runs a demo

`scripts/nova demo` does the following:
