...
└── 11_configurable/             # Phase 15 demo — DTB-configured guest, small vs large config
```
...
Guest configuration (Phase 15): the hypervisor's guest table is built
at boot from per-guest DTBs generated by `novakit.image.dtb` out of a
config YAML (`configs/*.yml`, selected with `./nova build --config`;
...
# Optional: guest config YAML the hypervisor must be built with
# (`./nova build --config`). Omitted = configs/default.yml.
...
4. Once `./nova demo verify NN_name` passes locally, flip `enabled: true` in the manifest. That commit marks the phase as complete.

## How the harness runs a demo

`./nova demo` does the following:
