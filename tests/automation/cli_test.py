...
class CliTests(unittest.TestCase):
    def completion(self, line: str):
        """What the shell would be handed for `line`, asked in-process.

        Click reads the completion variables when the command runs, so the
        answer is the same one a shell gets — and the process boundary is
        not what any of this is about. The wrapper that puts the command on
        a PATH is proven by the spawn below, once.
        """
        return RUNNER.invoke(
            cli.app,
            [],
            env={"_NOVA_COMPLETE": "complete_zsh", "_TYPER_COMPLETE_ARGS": line},
        )

    def test_root_serves_shell_completion_without_an_unmanaged_installer(self):
        offered = self.completion("nova ")
        self.assertEqual(offered.exit_code, 0, offered.stdout)
        self.assertIn("completion", offered.stdout)

        # Typer's own installer would write a startup file nothing here
        # tracks; `nova completion` owns that, so it must not be offered.
        listed = RUNNER.invoke(cli.app, ["--help"])
    ...
    def test_the_wrapper_finds_its_own_repository_and_python(self):
        ...
        self.assertIn("Build the hypervisor.", result.stdout)

    def test_demo_arguments_complete_from_the_manifest_catalog(self):
        for command in ("demo run ", "demo verify ", "workbench serve "):
            with self.subTest(command=command):
                offered = self.completion(f"nova {command}")

                self.assertEqual(offered.exit_code, 0, offered.stdout)
                self.assertNotEqual(offered.stdout.strip(), "_files")
                self.assertIn('"01_hello"', offered.stdout)
                self.assertIn('"18_panic_smoke"', offered.stdout)
                self.assertNotIn('\n"1"\n', offered.stdout)
    ...
    def test_completion_install_supports_any_directory_and_uninstall(self):
        with tempfile.TemporaryDirectory() as home:
            ...
            def installer(*args: str):
                """One `nova completion ...`, run from a directory that is
                not the repository — which is the whole claim: the paths
                come from `$HOME` and from the module's own location, never
                from where the command was typed."""
                with contextlib.chdir(away):
                    return RUNNER.invoke(
                        cli.app, ["completion", *args, "--shell", "zsh"], env=environment
                    )
            ...
            self.assertEqual(repeated.exit_code, 0, repeated.stdout)
                ...
                shell = subprocess.run(
                    [
                        ...
                        (
                            ...
                            # One run is what this proves: the command the
                            # startup file registered is on the PATH and
                            # works. That every subcommand answers for
                            # itself is asked in-process, of the registry.
                            "nova --help >/dev/null"
                        ),
                    ],
                    cwd=away,
            ...
            repeated_remove = installer("uninstall")
            self.assertEqual(repeated_remove.exit_code, 0, repeated_remove.stdout)
    ...
    def test_unknown_build_option_fails_before_build(self):
        # A parser refusal is not a process fact: what matters is that the
        # usage error arrives as exit 2 with the option named, before
        # anything configures or compiles.
        refused = RUNNER.invoke(cli.app, ["build", "--unknown"])

        self.assertEqual(refused.exit_code, 2)
        self.assertIn("No such option", refused.stderr)

    def test_build_selectors_are_mutually_exclusive(self):
        refused = RUNNER.invoke(cli.app, ["build", "--release", "--preset", "custom"])

        self.assertEqual(refused.exit_code, 2)
        self.assertIn("cannot be used together", refused.stderr)
