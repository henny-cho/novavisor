...
class RoundTripTest(unittest.TestCase):
    ...
    def test_the_extents_come_back_as_extents(self):
        """A symbol table read from JSON hands out lists unless the
        reader says otherwise, and a list is not the pair every caller
        unpacks."""
        self.write()
        restored = observe.load(self.artifact, self.elf)
        self.assertEqual(restored.symbols.extent_of("nova::vcpu::g_sched"), (0x4008_1000, 8))

    def test_the_view_sits_beside_the_image(self):
        self.assertEqual(self.artifact.parent, self.elf.parent)
        self.assertEqual(self.artifact.name, "novavisor.observe.json")
...
class AgainstTheImageTest(unittest.TestCase):
    ...
    def test_the_generator_writes_a_view_and_what_it_read(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(observe.load(out, ELF).enums, workbench_image.view().enums)
