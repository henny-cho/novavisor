...
class WindowRequestTest(unittest.IsolatedAsyncioTestCase):
    ...
    def bridge(self, stamps=range(0, 100)):
        ...
        bridge = support.bridge(trace_history=4096)
    ...
    async def test_a_third_question_supersedes_the_second_and_says_so(self):
        """One question per client per topic is answered at a time and one
        more waits. A third takes the waiting slot, and the one it
        displaced is refused by name — a client that dragged the window
        twice is otherwise left waiting for an answer nobody will build.
        """
        bridge = self.bridge()
        for number in (1, 2, 3):
            bridge._handle_uplink(json.dumps({
                "topic": "trace",
                "data": {"op": "window", "from": 0, "to": 9},
                "request_id": f"q-{number}",
            }))
        await bridge.settled()

        frames = bridge.store.drain()
        self.assertEqual(
            [frame["reply_to"] for frame in frames if frame["kind"] == "snapshot"],
            ["q-1", "q-3"],
        )
        self.assertEqual(
            [
            ...
            [("q-2", "trace: superseded by a newer request")],
        )
