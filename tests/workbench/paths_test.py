...
from novakit.services.workbench import events, hardware, paths
...
class PublishedEdgeTest(unittest.TestCase):
    ...
    def test_every_edge_carries_the_caption_it_is_drawn_under(self):
        # The row names itself and the UI falls back to the id, so a
        # missing caption reaches a reader as an internal word like
        # `inject` rather than as what the path is.
        for edge in self.edges:
            with self.subTest(edge=edge["id"]):
                self.assertTrue(edge["label"], f"{edge['id']} is published unnamed")
